...
import { Search, Filter, Eye, X, Loader2, Volume2, Download, Play } from "lucide-react";
...
const CallHistory = () => {
    ...
    const fetchCalls = async () => {
        try {
            ...
            const mappedCalls: Call[] = (data || []).map((c: any) => ({
                id: c.call_id || String(Math.random()),
                phone: c.caller_data?.phone_number || "Unknown",
                name: c.caller_data?.full_name || "Guest",
                date: c.started_at ? format(new Date(c.started_at), "yyyy-MM-dd HH:mm") : "N/A",
    ...
    const fetchSignedUrl = async (callId: string) => {
        try {
            setLoadingAudio(true);
            const res = await fetch(`${API_URL}/api/recordings/${callId}/url`);
            ...
            toast.error("Could not load recording. It might still be processing.");
        } finally {
    ...
    const handleDownload = async (callId: string) => {
        try {
            ...
            const res = await fetch(`${API_URL}/api/recordings/${callId}/url?download=true`);
    ...
    const filtered = calls.filter((c) => {
        ...
        if (outcomeFilter && c.outcome.toLowerCase() !== outcomeFilter.toLowerCase()) return false;
        return true;
    });
    ...
    return (
        <>
            ...
            <div className="p-6 page-transition space-y-4">
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-2 glass-card px-3 py-2 rounded-lg">
                        ...
                        >
                            ...
                            <option value="Callback" className="bg-card">Callback</option>
                        </select>
                    </div>
                    <button
                        onClick={fetchCalls}
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-border/30">
                                        {["Phone", "Name", "Date & Time", "Duration", "Outcome", "Actions"].map((h) => (
                                ...
                                <tbody>
                                    {filtered.length === 0 ? (
                                        <tr>
                                            <td colSpan={6} className="text-center py-20 text-muted-foreground">No call logs found.</td>
                                        </tr>
                                    ) : (
                                        filtered.map((call) => (
                                            <tr
                                            ...
                                            >
                                                ...
                                                <td className={`px-4 py-3 text-sm font-mono flex items-center gap-2 ${durationColor(call.durationSec)}`}>
                                                    {call.duration}
                                                    {call.recordingStatus === 'completed' && <Volume2 className="w-3.5 h-3.5 text-accent" />}
                                                </td>
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex gap-2">
                                                        ...
                                                        {call.recordingStatus === 'completed' && (
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); handleDownload(call.id); }}
                                                                disabled={downloadingId === call.id}
                                                                className="p-1.5 rounded-md hover:bg-accent/10 transition-colors text-muted-foreground hover:text-accent disabled:opacity-50"
                                                                title="Download Recording"
                                                            >
                                                                {downloadingId === call.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
                                                            </button>
                                                        )}
                                                    </div>
                                                </td>
                ...
                {/* Detail Modal */}
                {selected && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm" onClick={() => { setSelected(null); setAudioUrl(null); setPlayingId(null); }}>
                        <div className="glass-card rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto p-6 m-4" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <button onClick={() => { setSelected(null); setAudioUrl(null); setPlayingId(null); }} className="text-muted-foreground hover:text-foreground">
                                    <X className="w-5 h-5" />
                                </button>
                            </div>
                                ...
                                <div className="mb-6 p-4 bg-accent/5 border border-accent/20 rounded-xl">
                                    <div className="flex items-center justify-between mb-3">
                                        ...
                                        <button
                                            ...
                                            className="flex items-center gap-1 text-[10px] bg-accent/10 text-accent px-2 py-1 rounded hover:bg-accent/20 transition-colors disabled:opacity-50"
                                        >
                                            {downloadingId === selected.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Download className="w-3 h-3" />}
                                            Download MP3
                                        </button>
                                    </div>
                                    {playingId === selected.id && audioUrl ? (
                                        <audio controls autoPlay className="w-full h-10 custom-audio-player">
                                            <source src={audioUrl} type="audio/mpeg" />
                                            Your browser does not support the audio element.
                                        </audio>
                                    ) : (
                                        <button
                                            onClick={() => fetchSignedUrl(selected.id)}
                                            disabled={loadingAudio}
                                            className="w-full py-3 bg-accent text-white rounded-lg font-medium flex items-center justify-center gap-2 hover:bg-accent/90 transition-all shadow-glow disabled:opacity-50"
                                        >
                                            {loadingAudio ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4 fill-current" />}
                                            Listen to Recording
                                        </button>
                                    )}
                                </div>
