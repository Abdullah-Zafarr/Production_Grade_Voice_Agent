...
// Normalizes pathname to logical path (e.g., /admin/calls/ -> /calls)
function getLogicalPath(pathname: string): string {
  if (!pathname || pathname === "/" || pathname === "/admin" || pathname === "/admin/") return "/";
  let path = pathname.startsWith("/admin") ? pathname.slice(6) : pathname;
  ...
  return (
    ...
    >
      ...
      <div className="h-16 flex items-center gap-3 px-4 border-b border-border/30">
        <img src="/orb/logo/favicon.png?v=4" alt="Logo" className="w-8 h-8 rounded-lg shadow-glow" />
        {!collapsed && (
      ...
      <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto">
        {navItems.map((item) => {
          const active = logicalPath === item.path;
          return (
            <Link
              key={item.path}
              href={item.path}
